...
commands = {
    ...
    "rename" : "[Name] [NewName] | Renames entry",
...
def start(library):  
    ...
    while True:
        ...
        if len(arguments) < 0:
            ...
            result = commandAssignedFunctions[arguments[0]](arguments, library)
            if result == "exit":
                break
...
def save(arguments, library):
    global saved
    if len(arguments) != 2:
        print("You did not inclue the exact amount of info required for the save command.")
        return
